...
infrastructure/mysql/
  ...
  02-auth-schema.sql         # auth_db 建表（users）
  03-product-schema.sql      # product_db 建表（product，无categories表）
  04-points-schema.sql       # points_db 建表
  05-order-schema.sql        # order_db 建表
  06-seed-data.sql           # 种子数据
```

**注意**：product_db 只包含 product 表（表名为单数），不再有 categories 表。分类作为 product 表的 VARCHAR 字段存储。
